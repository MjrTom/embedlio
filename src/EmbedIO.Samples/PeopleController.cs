...
namespace EmbedIO.Samples
{
    ...
    public sealed class PeopleController : WebApiController
    {
        ...
        public static Task<IEnumerable<Person>> GetAllPeople() => Person.GetDataAsync();

        // Gets the first record.
        // This will respond to 
        //     GET http://localhost:9696/api/people/first
        [Route(HttpVerbs.Get, "/people/first")]
        public static async Task<Person> GetFirstPeople() => (await Person.GetDataAsync().ConfigureAwait(false)).First();
        ...
        public static async Task<Person> GetPeople(int id)
        ...
        public static async Task<GridDataResponse> PostPeople([JsonGridDataRequest] GridDataRequest gridDataRequest)
        ...
        public static Dictionary<string, object> Echo([FormData] NameValueCollection data)
            => data.ToDictionary();

        // Select by name
        [Route(HttpVerbs.Get, "/peopleByName/{name}")]
        public static async Task<Person> GetPeopleByName(string name)
